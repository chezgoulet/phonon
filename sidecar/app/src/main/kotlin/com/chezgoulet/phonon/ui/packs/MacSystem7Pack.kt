 ...
 *
 * Reactive elements respond to inference, battery, thermal, and health state
 * per the spec at chezgoulet/phonon#205.
 */
object MacSystem7Pack : VisualizationPack {

    override val id = "mac-system7"
    override val name = "Macintosh System 7"
    override val description = "Platinum desktop with Finder window, menu bar, trash, Happy/Sad Mac faces, and classic System Error bomb"
    override val author = "chezgoulet"
    override val version = "1.0.0"

    override val defaultConfig = mapOf(
        "platinum_brightness" to "1.0",  // 0.0–1.0: desktop background luminance
        "window_drift" to "true",         // enable slow Finder window drift
    )
    ...
    private val charMap = mapOf(
        ...
        'n' to intArrayOf(0x00, 0x3B, 0x04, 0x04, 0x04, 0x04, 0x04),
        ' ' to intArrayOf(0x00)
    )

    // ── Noise texture (static per activation) ────────────────────────
    ...
    override fun Render(state: VizState, modifier: Modifier) {
        ...
        LaunchedEffect(state.deviceId) {
            ...
            while (true) {
                ...
                dtSec = (tn - lastFrame).coerceIn(0.001f, 0.05f)
                lastFrame = tn
                tSec = tn
            }
        }
        ...
        Canvas(modifier = modifier.fillMaxSize()) {
            ...
            val isDegraded = state.lowPowerMode
            val isOverheating = state.batteryTemperature > 42f
            val isUnhealthy = !state.isHealthy
            val platinumBrightness = (state.themeConfig["platinum_brightness"] ?: "1.0").toFloatOrNull() ?: 1f

            // 1. Desktop background with noise
            drawRect(Color(0xFFD0D0D0))
            val n = noise
            val arr = noiseMap
            if (arr != null) {
                var idx = (n * 100).toInt()
                for (v in arr) {
                    ...
                    drawCircle(Color(0.92f + v, 0.92f + v, 0.92f + v).copy(alpha = platinumBrightness), 1f, Offset(bx, by))
                    idx++
                }
            }

            // 2. Menu bar
            drawMenuBar(w, t, state.isProcessing)

            // 3. Finder window(s)
            val nWin = if (load > 0.6f) 2 else 1
            drawFinderWindow(w, h, t, nWin, state.themeConfig["window_drift"]?.toBoolean() ?: true)

            // 4. Trash
            val papers = when { load > 0.6f -> 4; load > 0.3f -> 2; else -> 0 }
            drawTrash(w, h, t, load > 0.3f, papers)

            // 5. Chicago display number — inference load as percentage
            drawNumber(w, h, (load * 100).toInt().coerceIn(0, 100))

            // 6. System faces
            if (isUnhealthy) {
                drawFace(w, h, sad = true)
            } else if (load < 0.05f && !isDegraded && t.toLong().toInt() % 30 < 3) {
                drawFace(w, h, sad = false)
            }

            // 7. Bomb dialog (overheat)
            if (isOverheating && !isDegraded) {
                val phase = t % 5.0
                if (phase < 3.0) drawBombDialog(w, h, phase)
            }

            // 8. Cursor — slow drift across screen
    ...
    // ── Layer 2: Menu bar ───────────────────────────────────────────
    private fun DrawScope.drawMenuBar(w: Float, t: Float, processing: Boolean) {
        val bh = 18f
        drawRect(Color.White, Offset(0f, 0f), Size(w, bh))
        drawLine(textBlack, Offset(0f, bh), Offset(w, bh), 1f)

        // ⌘ symbol
        val cx = 12f
        drawLine(textBlack, Offset(cx + 7f, 3f), Offset(cx + 7f, 3f + 7f), 1f)
        drawLine(textBlack, Offset(cx, 3f + 3.5f), Offset(cx + 14f, 3f + 3.5f), 1f)
        drawLine(textBlack, Offset(cx + 3.5f, 3f), Offset(cx + 3.5f, 3f + 7f), 1f)

        val menus = listOf('F' to "File", 'E' to "Edit", 'V' to "View", 'S' to "Special")
        var x = 28f
        menus.forEachIndexed { i, (ch, _) ->
    ...
    private fun DrawScope.drawFinderWindow(w: Float, h: Float, t: Float, count: Int, allowDrift: Boolean) {
        val winW = w * 0.55f; val winH = h * 0.50f; val tbH = 18f
        val drift = if (allowDrift) Offset((sin(t / 60.0) * 8f).toFloat(), (cos(t / 90.0) * 4f).toFloat())
                    else Offset.Zero
        ...
        for (i in 0 until count) {
            ...
            // Title
            drawBitmapString("Phonon", bx + winW / 2f - 20f, by + 4f, textBlack)
        }
        // File icon in foremost window
        val bx = w * 0.22f + drift.x
        val by = h * 0.22f + drift.y
        val iy = by + tbH + 24f; val ix = bx + winW / 2f - 12f
        ...
        drawBitmapString("Phonon", ix - 4f, iy + 30f, textBlack)
        drawBitmapString("Kind: Audio Device", ix - 10f, iy + 46f, textBlack)
    }

    // ── Layer 4: Trash can ──────────────────────────────────────────
    private fun DrawScope.drawTrash(w: Float, h: Float, t: Float, full: Boolean, nPapers: Int) {
        ...
        val body = Path().apply {
            moveTo(tx - 5f + shake, ty); lineTo(tx + 29f + shake, ty)
            lineTo(tx + 22f + shake, ty + 32f); lineTo(tx + 2f + shake, ty + 32f); close()
        }
        drawPath(body, c, style = Stroke(1.5f))
        drawLine(c, Offset(tx - 7f + shake, ty - 4f), Offset(tx + 31f + shake, ty - 4f), 2f)
        if (full) repeat(3) { i ->
            drawLine(Color(0xFF888888), Offset(tx + 2f + shake, ty + 6f + i * 8f), Offset(tx + 22f + shake, ty + 12f + i * 8f), 1.5f)
        }
    }
    ...
    private fun DrawScope.drawNumber(w: Float, h: Float, v: Int) {
        ...
        val sx = w / 2f - (s.length * 7f) / 2f; val sy = h * 0.60f
    ...
    private fun DrawScope.drawFace(w: Float, h: Float, sad: Boolean) {
        val fs = 40f; val fx = w / 2f - fs / 2f; val fy = h / 2f - fs / 2f
        ...
        if (sad) {
            repeat(3) { drawLine(Color(0xFF888888), Offset(fx + 4f, fy + fs + 8f + it * 4f), Offset(fx + fs - 4f, fy + fs + 8f + it * 4f), 1f) }
        }
    }

    // ── Layer 7: Bomb dialog ────────────────────────────────────────
    private fun DrawScope.drawBombDialog(w: Float, h: Float, phase: Double) {
        val a = if (phase > 2.5f) (1f - (phase - 2.5f).toFloat() * 2f) else 1f
        val dw = w * 0.6f; val dx = w / 2f - dw / 2f; val dy = h / 2f - 50f
        ...
        drawBitmapStringNoShadow("Sorry, a system", dx + 50f, dy + 20f, textBlack.copy(a))
        drawBitmapStringNoShadow("error occurred.", dx + 50f, dy + 36f, textBlack.copy(a))
    }
    ...
    private fun DrawScope.drawBatteryIcon(w: Float, t: Float) {
    ...
    // ── Bitmap text helpers ─────────────────────────────────────────
    private fun DrawScope.drawBitmapString(text: String, x: Float, y: Float, c: Color) {
        var cx = x
        for (ch in text) {
    ...
    // Same as above but no shadow/font-weight ambiguity alias
    private fun DrawScope.drawBitmapStringNoShadow(text: String, x: Float, y: Float, c: Color) {
        drawBitmapString(text, x, y, c)
    }

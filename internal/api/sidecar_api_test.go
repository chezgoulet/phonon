...
func TestPair_Success(t *testing.T) {
	...
	w := execPost(t, mux, "/api/v1/sidecar/pair", body)
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", w.Code, w.Body.String())
	}
	...
	if resp.Status != "paired" {
		t.Errorf("expected status 'paired', got %q", resp.Status)
	}
	if resp.NodeName != "pixel-7a-0001" {
		t.Errorf("expected node name 'pixel-7a-0001', got %q", resp.NodeName)
	}
	if resp.PairID == "" {
		t.Error("expected non-empty pair_id")
	}
	...
	if node.State != registry.NodeStatePaired {
